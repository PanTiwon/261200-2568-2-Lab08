...
public class Turtle implements Runnable, ActionListener, MouseListener, MouseMotionListener, KeyListener, ComponentListener, MouseWheelListener
{
    ...
    {
        Gift gift1 = new Gift("Blue",0,0);
        Gift gift2 = new Gift("Green",250,150);
        Gift gift3 = new Gift("Black",-250,150);
        Gift gift4 = new Gift("Pink",-250,-150);
        Gift gift5 = new Gift("Purple",250,-150);
        // --- ส่วนที่แก้ไข ---

        // สร้าง Thread แยกสำหรับ gift แต่ละตัว
        Thread t1 = new Thread(gift1);
        Thread t2 = new Thread(gift2);
        Thread t3 = new Thread(gift3);
        Thread t4 = new Thread(gift4);
        Thread t5 = new Thread(gift5);
        // สั่งให้เริ่มทำงานพร้อมกัน (ต้องใช้ .start() ห้ามใช้ .run())
        t1.start();
        t2.start();
        t3.start();
        t4.start();
